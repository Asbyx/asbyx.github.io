...
function Vecteur(x, y) {
	this.x = x;
	this.y = y;
	this.norme = function () { // return the norm of the vector
		let norme;
		norme = Math.sqrt((this.x*this.x)+(this.y*this.y));
		return norme;
	};
	this.setNorme = function (n) { // set the norm of the vector
		let norme = this.norme();
		let x = this.x/norme; y = this.y/norme;
		this.x = x*n;
		this.y = y*n;
	};
	this.angle = function(){ //return the angle
		return Math.atan2(this.y, this.x); 
	}
	this.rotate = function(a){ //set the angle at a
		let norme = this.norme();
